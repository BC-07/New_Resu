from flask import Flask, request, render_template, jsonify, send_file, session, redirect, url_for, flash, g, make_response
...
from security_manager import SecurityManager, security_manager, require_api_key, obfuscate_response_data
from template_security import TemplateSecurityProcessor, template_processor
...
class PDSAssessmentApp:
    def __init__(self):
        ...
        self.template_processor = TemplateSecurityProcessor()
        
        # Set production mode (change this to False for development)
        self.production_mode = os.getenv('FLASK_ENV', 'production') == 'production'
    ...
    def _register_error_handlers(self):
        ...
        def not_found(error):
            ...
            # For authenticated users with invalid routes, redirect to dashboard with error message
            # This handles cases like /dashboard/invalid-section or /nonexistent-page
    ...
    def _register_security_middleware(self):
        """Register security middleware for request/response processing"""
        
        @self.app.before_request
        def before_request():
            # Store security manager in g for easy access
            g.security_manager = self.security_manager
            
            # Generate API key for this session if needed
            if 'api_key' not in session:
                session['api_key'] = self.security_manager.generate_dynamic_api_key(
                    session.get('user_id')
                )
        ...
        def after_request(response):
            ...
            response = self.security_manager.add_security_headers(response)
            
            # Process template content for security (only for HTML responses)
            if (response.content_type and 
                ...
                try:
                    ...
                    secured_content = self.template_processor.process_template(
                        html_content, 
                        self.production_mode
                    )
                    response.set_data(secured_content)
                except Exception as e:
                    logger.warning(f"Template security processing failed: {e}")
            
            return response
        
        @self.app.context_processor
        def inject_security_context():
            """Inject security-related variables into templates"""
            return {
                'session_token': session.get('csrf_token', self.security_manager.create_secure_session_token()),
                'api_key': session.get('api_key'),
                'production_mode': self.production_mode,
                'obfuscated_mode': self.production_mode
            }
    ...
    def _register_routes(self):
        ...
        # Secure static file handler for JavaScript
        self.app.add_url_rule('/static/js/<path:filename>', 'secure_js', self.serve_secure_js)
    ...
    def serve_secure_js(self, filename):
        """Serve JavaScript files with security processing"""
        try:
            # Build the file path
            js_path = os.path.join(self.app.static_folder, 'js', filename)
            ...
            # Apply security processing in production mode
            if self.production_mode:
                # Obfuscate JavaScript variables and functions
                js_content = self.security_manager.obfuscate_js_variables(js_content)
                
                # Minify JavaScript
                js_content = self.security_manager.minify_js(js_content)
            ...
            logger.error(f"Error serving JavaScript file {filename}: {e}")
        ...
        try:
            ...
            if self.production_mode:
                ...
                if not api_key or not self.security_manager.validate_api_key(api_key):
                    return jsonify({'error': 'Unauthorized access', 'code': 'SEC_001'}), 401
