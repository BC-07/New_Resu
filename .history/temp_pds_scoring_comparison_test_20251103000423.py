...
class PDSScoringComparisonTest:
    ...
    def calculate_score(self, pds_data: Dict, job_data: Dict) -> Dict:
        ...
        try:
            # Debug: Print PDS data structure
            ...
            print(f"      🔍 Debug - Job Requirements: {lspu_job['education_requirements'][:50] if lspu_job['education_requirements'] else 'None'}...")
            ...
            # Debug: Print assessment result structure
            if assessment_result:
                print(f"      🔍 Assessment Score: {assessment_result.get('percentage_score', 0)}")
                print(f"      🔍 Assessment Results: {assessment_result.get('assessment_results', {})}")
            
            # Extract scores from the assessment results structure
            percentage_score = assessment_result.get('percentage_score', 0)
            assessment_details = assessment_result.get('assessment_results', {})
            
            return {
                'total_score': percentage_score,
                'education_score': assessment_details.get('education', {}).get('score', 0),
                'experience_score': assessment_details.get('experience', {}).get('score', 0),
                'training_score': assessment_details.get('training', {}).get('score', 0),
                'eligibility_score': assessment_details.get('eligibility', {}).get('score', 0),
    ...
    def run_comparison_test(self):
        ...
        pds_data = {}
        for name, filename in list(self.test_files.items())[:1]:  # Test just first one for debugging
            try:
        ...
        job_data = {}
        for job_name, job_id in list(self.test_jobs.items())[:1]:  # Test just first one for debugging
            try:
