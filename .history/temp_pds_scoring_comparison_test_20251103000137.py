...
from assessment_engine import LSPUAssessmentEngine
from database import DatabaseManager

class PDSScoringComparisonTest:
    def __init__(self):
        self.extractor = ImprovedPDSExtractor()
        self.assessment_engine = LSPUAssessmentEngine()
    ...
    def extract_pds_data(self, filename: str) -> Dict:
        ...
        result = self.extractor.extract_from_excel(filepath)
        
        if not result.get('success'):
            raise Exception(f"Failed to extract from {filename}: {result.get('error')}")
    ...
    def calculate_score(self, pds_data: Dict, job_data: Dict) -> Dict:
        ...
        try:
            ...
            assessment_result = self.assessment_engine.assess_candidate(pds_data, lspu_job)
            
            return {
                'total_score': assessment_result.get('overall_score', 0),
