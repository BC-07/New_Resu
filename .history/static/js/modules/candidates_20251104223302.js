...
const CandidatesModule = {
    ...
    setLoadingState(isLoading) {
        ...
        // Show/hide loading state in content area
        if (this.candidatesContent && isLoading) {
            // Only show ONE type of loading indicator
            if (!this.candidatesData || Object.keys(this.candidatesData).length === 0) {
                ...
                this.candidatesContent.innerHTML = `
                    <div class="loading-state" data-loading-type="full">
                        <div class="loading-spinner">
                            <i class="fas fa-spinner fa-spin fa-3x text-primary"></i>
                        </div>
                        <p class="mt-3 text-muted">Loading candidates data...</p>
                        <small class="text-muted">Please wait while we fetch the latest candidate information.</small>
                    </div>
                `;
            } else {
                // Refreshing existing data - show subtle overlay
                const loadingIndicator = document.createElement('div');
                loadingIndicator.className = 'candidates-loading-overlay';
                loadingIndicator.setAttribute('data-loading-type', 'overlay');
                ...
                this.candidatesContent.appendChild(loadingIndicator);
            }
        }
    },
    ...
    clearAllLoadingStates() {
        ...
        // Remove overlay loading indicators
        const overlayIndicators = this.candidatesContent.querySelectorAll('.candidates-loading-overlay, [data-loading-type="overlay"]');
        overlayIndicators.forEach(indicator => indicator.remove());
        
        // Remove full loading states (but only if they're loading states, not actual content)
        const fullLoadingStates = this.candidatesContent.querySelectorAll('[data-loading-type="full"]');
        fullLoadingStates.forEach(state => state.remove());
        
        // Hide any global loading overlays that might be active
        if (window.LoadingUtils && typeof window.LoadingUtils.hide === 'function') {
